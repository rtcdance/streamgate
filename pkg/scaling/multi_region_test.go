...
func TestMultiRegionManager_RegisterRegion(t *testing.T) {
	mrm := NewMultiRegionManager(30 * time.Second)
...
func TestMultiRegionManager_SetPrimaryRegion(t *testing.T) {
	...
	region1 := &Region{ID: "us-east-1", Name: "US East", Active: true}
	region2 := &Region{ID: "eu-west-1", Name: "EU West", Active: true}
...
func TestMultiRegionManager_ShouldHealthCheck(t *testing.T) {
	mrm := NewMultiRegionManager(1 * time.Millisecond)
