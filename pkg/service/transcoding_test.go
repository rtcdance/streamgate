...
func TestTranscodingService_Transcode(t *testing.T) {
	...
	taskID, err := service.Transcode("content-1", "720p", "https://example.com/input.mp4", 5)
...
func TestTranscodingService_TranscodeRejectsInvalidProfile(t *testing.T) {
	...
	_, err := service.Transcode("content-1", "144p", "https://example.com/input.mp4", 1)
	require.Error(t, err)
...
func TestTranscodingService_InMemoryStatusFlow(t *testing.T) {
	...
	taskID, err := service.Transcode("content-2", "1080p", "https://example.com/input.mp4", 7)
	require.NoError(t, err)

	task, err := service.GetTranscodingStatus(taskID)
	require.NoError(t, err)
	assert.Equal(t, "pending", task.Status)
	assert.Equal(t, 0, task.Progress)

	require.NoError(t, service.StartTask(taskID))
	task, err = service.GetTranscodingStatus(taskID)
	require.NoError(t, err)
	assert.Equal(t, "processing", task.Status)
	require.NotNil(t, task.StartedAt)

	require.NoError(t, service.UpdateTaskProgress(taskID, 45))
	task, err = service.GetTranscodingStatus(taskID)
	require.NoError(t, err)
	assert.Equal(t, 45, task.Progress)

	require.NoError(t, service.CompleteTask(taskID, "https://example.com/output.m3u8"))
	task, err = service.GetTranscodingStatus(taskID)
	require.NoError(t, err)
	...
	list, err := service.ListTasks("content-2", 10, 0)
	require.NoError(t, err)
	require.Len(t, list, 1)
	assert.Equal(t, taskID, list[0].ID)

	pending, err := service.GetPendingTasks(10)
	require.NoError(t, err)
...
func TestTranscodingService_CancelAndDeleteFallback(t *testing.T) {
	...
	taskID, err := service.Transcode("content-3", "480p", "https://example.com/input.mp4", 3)
	require.NoError(t, err)

	require.NoError(t, service.CancelTask(taskID))
	task, err := service.GetTranscodingStatus(taskID)
	require.NoError(t, err)
	assert.Equal(t, "cancelled", task.Status)
	require.NotNil(t, task.CompletedAt)

	require.NoError(t, service.DeleteTask(taskID))
	_, err = service.GetTranscodingStatus(taskID)
	require.Error(t, err)
...
func TestTranscodingService_ListTasksFilterAndPagination(t *testing.T) {
	...
	id1, err := svc.Transcode("content-a", "720p", "https://example.com/a1.mp4", 1)
	require.NoError(t, err)
	id2, err := svc.Transcode("content-a", "480p", "https://example.com/a2.mp4", 2)
	require.NoError(t, err)
	id3, err := svc.Transcode("content-b", "1080p", "https://example.com/b1.mp4", 3)
	require.NoError(t, err)

	filtered, err := svc.ListTasks("content-a", 10, 0)
	require.NoError(t, err)
	require.Len(t, filtered, 2)
	assert.Equal(t, "content-a", filtered[0].ContentID)
	assert.Equal(t, "content-a", filtered[1].ContentID)

	paged, err := svc.ListTasks("", 1, 1)
	require.NoError(t, err)
	require.Len(t, paged, 1)
	assert.Contains(t, []string{id1, id2, id3}, paged[0].ID)

	emptyPage, err := svc.ListTasks("", 2, 10)
	require.NoError(t, err)
