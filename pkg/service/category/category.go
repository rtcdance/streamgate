...
func (s *CategoryService) GetCategory(ctx context.Context, id string) (*models.Category, error) {
	...
	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("category not found %s: %w", id, serviceerrors.ErrNotFound)
	}
	if err != nil {
...
func (s *CategoryService) UpdateCategory(ctx context.Context, cat *models.Category) error {
	...
	if rows == 0 {
		return fmt.Errorf("category not found %s: %w", cat.ID, serviceerrors.ErrNotFound)
	}
	return nil
}
...
func (s *CategoryService) DeleteCategory(ctx context.Context, id string) error {
	...
	rows, _ := result.RowsAffected()
	if rows == 0 {
		return fmt.Errorf("category not found %s: %w", id, serviceerrors.ErrNotFound)
	}
	return nil
}
