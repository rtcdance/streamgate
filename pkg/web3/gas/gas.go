...
// EIP1559Levels is a slice of GasPrice pointers for EIP-1559 fee estimation.
...
func (fe *FeeHistoryEstimator) EIP1559GasLevels(ctx context.Context) ([]*GasPrice, error) {
	...
	levels := EIP1559Levels{
		{Level: "safe", GasPrice: safeMaxFee, Gwei: toGwei(safeMaxFee), EstimatedTime: estimatedTimes[0], BaseFee: predictedBaseFee, MaxPriorityFeePerGas: lowTip, MaxFeePerGas: safeMaxFee},
		{Level: "standard", GasPrice: standardMaxFee, Gwei: toGwei(standardMaxFee), EstimatedTime: estimatedTimes[1], BaseFee: predictedBaseFee, MaxPriorityFeePerGas: medTip, MaxFeePerGas: standardMaxFee},
		{Level: "fast", GasPrice: fastMaxFee, Gwei: toGwei(fastMaxFee), EstimatedTime: estimatedTimes[2], BaseFee: predictedBaseFee, MaxPriorityFeePerGas: highTip, MaxFeePerGas: fastMaxFee},
	}
...
func (fe *FeeHistoryEstimator) fallbackLevels(ctx context.Context) ([]*GasPrice, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		gasPrice, lastErr = fe.provider.GetGasPrice(ctx)
		if lastErr == nil {
			break
		}
		fe.logger.Warn("fallback GetGasPrice attempt failed",
	...
	if lastErr != nil {
		return nil, fmt.Errorf("failed to get gas price for fallback after 3 attempts: %w", lastErr)
	}
...
type GasMonitor struct {
	client         GasPricer
	feeEstimator   *FeeHistoryEstimator
	logger         *zap.Logger
...
func (gm *GasMonitor) Start(ctx context.Context) error {
	gm.logger.Info("Starting gas monitor", zap.Duration("update_interval", gm.updateInterval))
