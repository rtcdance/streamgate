...
func TestMemoryEventStore_Dedup(t *testing.T) {
	...
	// Second save should fail (duplicate)
	err = store.SaveEvent(event)
	assert.Error(t, err)
...
func TestMemoryEventStore_MarkEventsReorged(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, 2, count) // e1 and e3 have blockHash 0xaaa

	// Verify events are marked
...
func TestEventIndexer_WithReorgDetector(t *testing.T) {
	...
	// Verify reorg detector can be set
	rd := &ReorgDetector{}
	indexer.SetReorgDetector(rd)
	assert.NotNil(t, indexer.reorgDetector)
}
...
func TestEventIndexer_ResumeFromCheckpoint(t *testing.T) {
	...
	// Simulate the Start() checkpoint loading logic
