...
// It performs: ABI-encode calldata → nonce acquisition → gas estimation →
...
func (cw *ContractWriter) SendTx(ctx context.Context, opts ContractTxOpts) (*ContractWriteResult, error) {
	// 1. Encode calldata
	parsedABI := opts.ParsedABI
	if parsedABI == nil {
		return nil, fmt.Errorf("contract_writer: ParsedABI is required")
	}

	callData, err := parsedABI.Pack(opts.Method, opts.Args...)
	if err != nil {
	...
	if gasLimit == 0 {
		...
		if err != nil {
			// Fallback: use a generous default
			gasLimit = 300_000
	...
	err = cw.key.UseKey(func(privKey *ecdsa.PrivateKey) error {
		...
		if err != nil {
			...
			if err != nil {
				return fmt.Errorf("contract_writer: sign legacy tx: %w", err)
			}
		...
		if err != nil {
			return fmt.Errorf("contract_writer: sign EIP-1559 tx: %w", err)
		}
	...
	// 5. Send transaction (no failover — prevents duplicate submission)
	...
	// 6. Optionally track for gas bumping
	if cw.tracker != nil {
