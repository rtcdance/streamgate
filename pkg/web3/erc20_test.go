...
// erc20MockCaller implements EthCaller for unit testing ERC20Reader.
type erc20MockCaller struct {
	responses map[string][]byte // method selector hex → response data
}
...
func TestERC20Reader_GetTokenInfo_MockError(t *testing.T) {
	...
	// GetTokenInfo is best-effort: returns defaults when calls fail
	if err != nil {
	...
	// When decimals() fails, it defaults to 18
	if info.Decimals != 18 {
