...
// ERC20ABI is the standard ERC-20 function ABI for read operations.
...
// PermitABI is the minimal ABI for the ERC-20 permit function (EIP-2612).
...
// ERC20Reader provides read-only access to ERC-20 token contracts.
...
// NewERC20Reader creates a new ERC-20 token reader.
func NewERC20Reader(caller EthCaller, logger *zap.Logger) *ERC20Reader {
	return &ERC20Reader{
		caller: caller,
		logger: logger,
		abi:    mustParseABI("ERC-20", ERC20ABI),
	}
}

// ERC20TokenInfo holds ERC-20 token metadata.
...
// GetTokenInfo retrieves name, symbol, decimals, and totalSupply for an ERC-20 token.
...
func (r *ERC20Reader) callUint8(ctx context.Context, contract common.Address, method string) (uint8, error) {
	...
	if len(out) > 0 {
		// ABI returns uint8 as uint8 or *big.Int depending on go-ethereum version
		switch v := out[0].(type) {
		case uint8:
			return v, nil
		case *big.Int:
...
// GetPermitNonce returns the current EIP-2612 permit nonce for the given owner address.
// This is required to construct a valid EIP-712 typed data for gasless permit signing.
...
// PackPermitCall encodes the EIP-2612 permit function call data.
// This returns the ABI-encoded call data that can be sent as a transaction
// to the ERC-20 contract's permit(address, address, uint256, uint256, uint8, bytes32, bytes32) function.
