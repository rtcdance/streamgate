...
func TestEventIndexer_StartStop(t *testing.T) {
	...
	// Verify it's running by checking current block advances
...
func TestEventIndexer_Dedup(t *testing.T) {
	...
	// Add same event twice via processLog
	ctx := context.Background()
	ei.processLog(ctx, log)
	ei.processLog(ctx, log) // duplicate
...
func TestEventIndexer_ConfirmationBlocks(t *testing.T) {
	...
	// Add logs at blocks 18-20 with confirmationBlocks=2
	// Only blocks up to 20-2=18 should be indexed
	logs := []types.Log{
	...
	events := ei.GetEvents()
	// With blockNum=20 and confirmationBlocks=2, safeBlock=18
	// Only blocks 16,17 should be indexed (currentBlock starts at 0, advances)
	// The actual behavior depends on currentBlock state
	if len(events) == 0 {
...
func TestEventIndexer_WSDisconnectSwitchToPolling(t *testing.T) {
	...
	// Create a channel and close it to simulate WS disconnect
	logsCh := make(chan types.Log)
	close(logsCh) // immediately close

	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()

	// websocketLoop should detect the closed channel and signal mode switch
	ei.wg.Add(1)
	...
	// Mode should have been signaled as polling
	select {
	...
	_ = mode // mode field itself may not have changed yet (that's indexingLoop's job)
}
