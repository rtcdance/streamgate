...
func main() {
	// Initialize logger
	log := logger.NewDevelopmentLogger("streamgate-auth")
	defer func() { _ = log.Sync() }()

	log.Info("Starting StreamGate Auth Service...", zap.String("version", Version))
	...
	if err := kernel.RegisterPlugin(auth.NewAuthPlugin(cfg, log)); err != nil {
		log.Fatal("Failed to register auth plugin", zap.Error(err))
	}
	...
	log.Info("StreamGate Auth Service started successfully", zap.Int("port", cfg.Server.Port))
	...
	log.Info("StreamGate Auth Service stopped gracefully")
}